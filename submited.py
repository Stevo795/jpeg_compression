    ...
    for i in range( 0, Nrows, blockSize):
        ...
        for j in range( 0, Ncols, blockSize ): # Apply to block starting at i,j
            for k in range(3):                 # Apply to channel Y (k=0), Cb (k=1), or Cr (k=2)
                ...
                for u in range(0, 8):
                    for v in range(0, 8):
                        # Obtain the sum by multiplying two numpy array and then apply numpy.sum.
                        # The first matrix is an 8x8 matrix of product of
                        # cos(((2x+1)*u*pi)/(2*blocksize)) * cos(((2x+1)*v*pi)/(2*blocksize)).
                        # This matrix is obtained by dot product of the two cosine parts.
                        # The other matrix is a 8x8 subset of the Image. This matrix is obtained
                        # by slicing the Image.
                        ax = np.cos(((2 * np.array([[i for i in range(blockSize)]]) + 1) * u * np.pi) / (2*blockSize))
                        bx = np.cos(((2 * np.array([[i for i in range(blockSize)]]) + 1) * v * np.pi) / (2*blockSize))
                        cos_res = np.dot(ax.T, bx)
                        sum_res = np.sum(inputImage[i:i + blockSize, j:j + blockSize, k] * cos_res)
                        #Obtain the value of function alpha(u) and alpha(v)
                        alpha_u = 1 / (np.sqrt(2)) if u == 0 else 1
                        alpha_v = 1 / (np.sqrt(2)) if v == 0 else 1
                        #Obtain the dct value by multiply them together, and assign the value
                        # to corresponding dct matrix position.
                        dct[u][v] = (1 / np.sqrt(2 * blockSize)) * alpha_u * alpha_v * sum_res
                ...
                # Apply quantization to dct matrix by dividing quantization matrix with
                # compression factor. As the compression factor increases, the compression
                # level increases.
                # The k below represents 3 different channels.
                tmp = np.round(dct / (quantizationTable[k] * compressionFactor))
                ...
                # YOUR CODE HERE [1 mark]
                # Record DC component by assigning the element at position (0,0)
                # to the DCencoding array. the first index of DCencoding is the
                # kth channel. The second index is the position index of the
                # DC component. Every time after adding a new element, the
                # DCencodingIndex is added up by 1, so the point will move to
                # the next empy position.
                # The k below represents 3 different channels.
                DCencoding[k][DCencodingIndex[k]] = tmp[0][0]
                DCencodingIndex[k] += 1
                ...
                # Record AC component into the ACencoding array with a
                # zig-zag manner. This will iterate over the dct matrix
                # and pick a value at each time. The picked value will
                # be record in the ACencoding matrix at the position of
                # it's corresponding position in a zig-zag scan manner.
                # (Which is the order in the zig-zag matrix). Since
                # the element at (0,0) is the DC component, (0,0) is
                # skipped.

                # The ACencodingIndex is the pointer index of the array.
                # Every time blocksize^2 - 1 value added in to the ACencoding
                # array, the pointer index moves blocksize^2 - 1 position
                # forward.
                for a in range(0, blockSize):
                    for b in range(0, blockSize):
                        if a == 0 and b == 0:
                            continue
                        ACencoding[k][ACencodingIndex[k] + zigzag[a][b] - 1] = tmp[a, b]
    ...
    for i in range(0, Nrows, blockSize):
        ...
        for j in range(0, Ncols, blockSize): # Apply to block starting at i,j
            for k in range(3):               # Apply to channel Y (k=0), Cb (k=1), or Cr (k=2)
                ...
                # YOUR CODE HERE [1 mark]
                # Extract AC components by iterating over the ACencoding array and
                # pick one value at a time. When the elements were stored in the
                # ACencoding array, a zig-zag manner is applied. So when returning
                # the value to dct matrix, we should find the value at its corresponding
                # zig-zag position zigzag[a,b] (in the ACencoding array).
                #
                # Every time blocksize^2 - 1 AC components extracted, the array
                # pointer moves blocksize^2 - 1 forward. So the ACencodingIndex
                # is added up by blocksize^2 - 1.
                # The k below represents 3 different channels. (Y Cr Cb)
                for a in range(0, blockSize):
                    for b in range(0, blockSize):
                        if a == 0 and b == 0:
                            continue
                        dct[a, b] = ACencoding[k][ACencodingIndex[k] + zigzag[a, b] - 1]
                ACencodingIndex[k] += (blockSize * blockSize - 1)
                ...
                # Similarly, extract DC components from DCencoding array.
                # Put the element to [0,0] position of the dct matrix.
                # The array index is moved 1 position forward every time
                # one element is extracted.
                #
                # The k below represents 3 different channels. (Y Cr Cb)
                ...
                # De-quantization of the dct matrix.
                # The result is obtained by multiplying the quantized
                # dct matrix by quantizationTable*compressionFactor.
                # The factor here is same as the factor in the
                # matrix quantization step above.
                tmp = dct * (quantizationTable[k]*compressionFactor)
                ...
                # YOUR CODE HERE [1 mark]
                # Obtain the inverse DCT by iterating over the subset
                # of the Image, and multiplying dct values by
                # cos(((2x+1)*u*pi)/(2*blocksize))*cos(((2y+1)*u*pi)/(2*blocksize)).
                # The coefficients and cosine parts are calculated in the dctBases.
                # dctBases[:,:,x % blockSize,y % blockSize] is the 8x8 matrix of
                # u * v for all u, v, and specific x value and y value.
                # So we can multiply the de-quatized dct matrix by the subset
                # of dctBases, and obtain the result by sum up the product matrix.
                # Then the result is stored to its corresponding position in the
                # image.
                for x in range(i, i + blockSize):
                    for y in range(j, j + blockSize):

                        sum_res=np.sum(tmp*dctBases[:,:,x % blockSize,y % blockSize])

                        image[x, y, k] = np.round(sum_res)
