...
def forwardJPEG():
    ...
    print(DCencoding.shape)
    print(ACencoding.shape)
    print(f"input image: {inputImage.shape}")
    ...
    for i in range(0, Nrows, blockSize):
        ...
        for j in range(0, Ncols, blockSize):  # Apply to block starting at i,j
            ...
            for k in range(3):  # Apply to channel Y (k=0), Cb (k=1), or Cr (k=2)
                ...
                #
                #             print(f"i:{i}  j:{j}  k:{k}")
                #             for u in range(i, i + blockSize ):
                #                 for v in range(j, j + blockSize ):
                #                     lista.append(inputImage[u, v, k])
                #             print(lista)
                # pass
                ...
                for u in range(0, 8):
                    ...
                    for v in range(0, 8):
                        ...
                        inputImage[i:i+blockSize, j:j+blockSize, k]

                        ax = np.cos(((2 * np.array([[i for i in range(blockSize)]]) + 1) * u * np.pi) / 16.0)
                        bx = np.cos(((2 * np.array([[i for i in range(blockSize)]]) + 1) * v * np.pi) / 16.0)
                        ...
                        #         sum += inputImage[x, y, k] * np.cos(((2.0 * x + 1) * u * np.pi) / 16.0) * np.cos(
                        #             ((2.0 * y + 1) * v * np.pi) / 16.0)
                        # if k == 0:
                        #     print(f"u:{u} v: {v}  sum: {sum}")

                        alpha_u = 1 /(np.sqrt(2)) if u == 0 else 1
                        alpha_v = 1 / (np.sqrt(2)) if v == 0 else 1

                        dct[u][v] = (1/4) * alpha_u * alpha_v * sum_res



                # <--------------------------------------------------->
                # <--at this point, the dct of block i,j is obtained
                # print(f"temp:{dct[0][0]} de:{quantizationTable[k][0][0]} res: {tmp[0][0]}")
                # <--------------------------------------------------->
                ...
                # if k==0:
                #     print(f"index{DCencodingIndex[k] }  tmp: {dct[0][0]}")
    ...
    for i in range(0, Nrows, blockSize):
        ...
        for j in range(0, Ncols, blockSize):  # Apply to block starting at i,j
            for k in range(3):  # Apply to channel Y (k=0), Cb (k=1), or Cr (k=2)
                ...
                ACencodingIndex[k] += 63 #(blockSize * blockSize - 1)
                ...
                for x in range(i, i + blockSize):
                    for y in range(j, j + blockSize):
                        ...
                        # check2=dctBases[:][:][x % blockSize][y % blockSize]
                        ...
                        # #         #tmp is dct after de-quantization
                        #
                        #         sum += tmp[u, v] * dctBases[u][v][x % blockSize][y % blockSize]

                                # if k == 0:
                                #     # print(f"x:{x} y:{y} u:{u} v:{v}")
                                #     print(f"u: {beta_u}  v:{beta_v} sum: {sum}")
                                #     print(f"*{dctBases[u][v][x % 8][y % 8]}")
                                #     print(dct)
                        ...
                        # #     print(f"*{dctBases[u][v][x % 8][y % 8]}")
                        # #     print(dct)
                        # beta_u = 2 if u == 0 else 1
                        # beta_v = 2 if v == 0 else 1
                        # image[x, y, k] = np.round(beta_u*beta_v*  sum)
        ...
        for v in range(blockSize):
            ...
            for x in range(blockSize):
                for y in range(blockSize):
                    dctBases[u][v][x][y] = 0.25 * alphaU * alphaV * np.cos(
                        (2 * x + 1) * u * PI / (2 * blockSize)) * np.cos((2 * y + 1) * v * PI / (2 * blockSize))
...
def display():
    ...
    glBindTexture(GL_TEXTURE_2D, texID)
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, outputImage.shape[1], outputImage.shape[0], 0, GL_RGB, GL_UNSIGNED_BYTE,
                 imgData)
